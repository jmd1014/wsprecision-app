...
- **수주 = 진행 의사 (2026-09-01 사용자 확정)**: 미납 수주가 미등록·휴면 제품에
  걸리면 수주 관리 상단 "제품 확인 필요" 패널에서 조치 — 휴면은 단가·소재비 변동을
  보여준 뒤 활성화(단가 갱신 시 master_change_log), 미등록은 기존 제품 매핑
  (customer_part_mapping 저장 → 업로드 매칭이 우선 참조) 또는 수주 정보만으로 최소
  등록(BOM·라우팅은 BOM 편집에서). 업로드 매칭은 활성 제품 우선
