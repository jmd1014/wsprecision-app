...
- **배치 상태 기계 (2026-08-27 사용자 확정)**: 배치 위치 = 공정 + `step_status`(WAIT 투입
  대기/RUN 진행 중, Migration 049). 공정마다 [투입](대기→진행)·[완료](→다음 공정 대기),
  사내 공정은 [투입+완료 한번에] 보조. 외주는 출고=투입·입고=완료, 생산(MES)은 투입 후
  완료 인수, 검사는 투입 후 판정([전량 합격] 퀵 + 상세)이 완료. **[공정 취소] = 배치의
  마지막 이벤트가 그 공정 투입일 때만**(이후 처리·분기 시 차단 — 추적성 보호), 역이벤트
  STEP_CANCEL 기록. 스테퍼: 소재입고 칸 폐지, 지나간 공정=초록·진행=파랑·대기=주황
  테두리(.step.wait). 합치기는 같은 공정·위치·상태끼리만. 라우팅 데이터의 MAT_IN 은
  호환용으로 저장 시 자동 삽입(표시 안 함)
