...
- **설비 스케줄 보드 1단계** (2026-09-07 목업 승인 후 구현, Migration 057,
  ...
  선택하면 보드에 가능 설비 빗금. 오른쪽 **보드** = HTML 표(.sb-* CSS, 전역 블록)
  — 실적은 생산일정 시트 행이 같은 설비·날짜·교대·작업 키로 붙어 계획→진행→
  달성/미달 색, 비가동 행은 정지사유 칩, 계획 없는 실적은 점선 칩. 셀 편집은
  st.form 하나로 기간 × 교대 일괄 저장(수량 기본 = 가동시간 × 표준 UPH − 준비시간,
  야간 미운영 설비(`machines.night_active`)는 야간 건너뜀), 셀 비우기는 expander.
