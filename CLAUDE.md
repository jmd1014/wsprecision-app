...
- **BOM 하위 탭 = 리스트→상세 카드** (2026-08-20 확정, 일괄 편집 표 제거): 행 선택 →
...
- **라우팅 편집 = 순서 배열만** (2026-08-20 확정): st.multiselect 클릭 순서 = 공정 순서.
  기본 4공정(소재입고·생산·검사·완성) 필수, 소재입고 첫·완성 끝 검증. 외주 스텝의
  default_vendor 는 BOM 공정행의 매칭 업체를 저장 시 복사 (라우팅에서 거래처 편집 없음 —
