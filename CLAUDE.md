...
- **설비 스케줄 보드 1단계** (2026-09-07 목업 승인 후 구현, Migration 057,
  ...
  쓰기 어렵다"): [계획 입력] = 설비 × 날짜·교대 격자 data_editor, 셀마다
  SelectboxColumn 으로 작업 선택(라벨 "작업 · 납기 · 남은 n", 타이핑 검색, 빈 값 =
  비움) → [보드 저장] 한 번에 upsert/delete, 수량 = 표준 UPH × 교대 시간 − 준비시간
  자동. [보드 보기] = HTML 표(.sb-* CSS, 전역 블록) — 실적은 생산일정 시트 행이
  같은 설비·날짜·교대·작업 키로 붙어 계획→진행→달성/미달 색, 비가동 행은 정지사유
  칩, 계획 없는 실적은 점선 칩. 장비군 필터로 행 축소. 기간 일괄 입력·수량/메모/
  작업지시 조정(비움 체크)은 expander. 야간 미운영 설비(`machines.night_active`)는
  야간 칸 저장을 건너뜀.
