...
- **라우팅 편집 = 같은 화면 하단, 순서 배열만** (2026-08-20 확정): multiselect 로 공정
  구성 선택 + **streamlit-sortables 드래그로 순서 조정**(2026-08-21, 미탑재 시 클릭 순서
  폴백 — try/except + 집합 동일성 검증). 번호 플로우 미리보기(파란 칩=BOM 공정,
  업체명/미매칭 병기). 기본 4공정(소재입고·생산·검사·완성) 필수, 소재입고 첫·완성 끝
  검증. 외주 스텝의 default_vendor 는 BOM 공정행의 매칭 업체를 저장 시 복사
  (업체 변경 후에는 라우팅 재저장 필요)
