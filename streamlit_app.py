...
def toss_grid_edit(rows, *, key, editable_cols=(), num_cols=(), strong_cols=(),
    ...
    try:
        ...
        _num_parse = JsCode("""
            function(p){ const n = Number(p.newValue);
        ...
        gb.configure_default_column(resizable=True, sortable=False,
                                    ...
                                    flex=1, minWidth=88, editable=False)
        ...
        gb.configure_grid_options(
            ...
            suppressColumnVirtualisation=True, suppressRowVirtualisation=True,
            scrollbarWidth=10,
        ...
        # reload_data=True: rerun 마다 새 DataFrame 을 그리드에 다시 넣는다 —
        ...
        if _data is not None:
            _recs = (_data.to_dict("records") if hasattr(_data, "to_dict")
                     else list(_data))
            for rec in _recs:
                try:
                    _ix = int(rec.get("_i"))
        ...
        if _sel is not None:
            ...
            if _srecs and _srecs[0].get("_i") is not None:
                _ix = int(_srecs[0]["_i"])
                _sel_ix = _ix if 0 <= _ix < len(rows) else None
        return _out, _sel_ix
    except Exception:
        ...
        for _bi, _brow in _ed.iterrows():
            if int(_bi) < len(_out):
                for c in editable_cols:
                    _out[int(_bi)][c] = _brow.get(c)
        return _out, (0 if rows else None)
    ...
    try:
        ...
        for c in _cols:
            ...
            if _kw:
                gb.configure_column(c, **_kw)
        gb.configure_selection("single")
        # 12행 이하는 전체 펼침, 그 이상은 고정 높이 + 리스트 내부 스크롤
        # (2026-09-03 사용자 확정: 전체 펼침은 페이지가 너무 길어 불편).
        # 내부 스크롤이 죽던 원인은 별도로 잡았다 — iframe·컨테이너 폭
        # 강제(CSS), 스크롤바 폭 명시, 세로 스크롤바 항상 표시
        _auto = height is None and len(rows) <= 12
        gb.configure_grid_options(
...
if page == "홈":
    ...
    if _open_ids:
        # 120건 상한을 없애고 100건씩 나눠 조회 (2026-09-16 리뷰: 미납
        # 수주가 많으면 집계가 조용히 빠지던 문제)
        _id_chunks = [",".join(str(i) for i in _open_ids[_i0:_i0 + 100])
                      for _i0 in range(0, len(_open_ids), 100)]
        try:
            _h_lines = []
            for _ids in _id_chunks:
                _h_lines += fetch("sales_order_items",
                    ...
                    "&order=pending_qty.desc", limit=1000)
        ...
        try:
            _h_sc_rows = []
            for _ids in _id_chunks:
                _h_sc_rows += fetch("so_delivery_schedule",
                    "so_id,soi_id,due_date,qty,delivered_qty",
                    f"so_id=in.({_ids})&order=due_date.asc", limit=1000)
            for _sc in _h_sc_rows:
    ...
    with tab1:
        ...
        with st.expander("신규 거래처 등록"):
            with st.form("m_new_vendor_form"):
                ...
                _m_new_go = st.form_submit_button("신규 등록", type="primary")
                ...
                if not (_md_name or "").strip():
                    ...
                    if "raw_name" in _mupd:
                        try:
                            ...
                            _sync_err = str(_e_sync)
                    if _sync_err:
                        st.warning(f"{_mid} 자재는 저장됐지만 BOM·제품 표기 "
                                   f"동기화에 실패했습니다: {_sync_err} — "
                                   "정합 점검에서 확인하세요.")
                    else:
        ...
        with st.expander("신규 자재 등록"):
            ...
            with st.form(f"nm_form_{_mat_prefix}"):
                ...
                _nm_force = st.checkbox(
                    ...
                    key="nm_force")
                _nm_go = st.form_submit_button("자재 등록", type="primary")
            if _nm_go and not (_nm_name or "").strip():
                st.error("자재명을 입력하세요.")
            elif _nm_go:
                _nn = _nm_name.strip()
                try:
                ...
                for _k, _v in sorted(_dups.items()):
                    ...
                    with st.expander("{}  ({}건)".format(_lab, len(_v))):
                        ...
                        if st.session_state.get(f"cfm_mg_go_{_kk}") \
                            ...
                            _nbom = 0
                            # 되돌리기 목록 — 단계마다 역연산을 쌓고, 중간
                            # 실패 시 역순으로 실행 (2026-09-16 리뷰: 부분
                            # 이동 상태로 남던 문제)
                            _undo = []
                            try:
                                for m in _drop:
                                    ...
                                    for b in _db.fetch("bom", "*",
                                        ...
                                        if b["product_id"] in _keep_pids:
                                            ...
                                            _nbom += 1
                                    # 원장·발주라인·매입매핑 — 옮길 행 id 를
                                    # 먼저 적어 두고 그 행만 되돌린다
                                    for _tbl, _pk, _col in (
                                            ("inventory_transactions", "txn_id",
                                             "material_id"),
                                            ("purchase_order_items", "poi_id",
                                             "material_id"),
                                        ...
                                        for _i0 in range(0, len(_ids), 100):
                                            ...
                                            _flt = f"{_pk}=in.(" + ",".join(
                                                str(x) for x in _chunk) + ")"
                                            _db.update(_tbl, _flt, {_col: _kid})
                                            _undo.append(("update", _tbl, _flt,
                                                          {_col: _did}))
                                ...
                                _n_undo, _n_fail = 0, 0
                                for _op, _tbl, _flt, _data in reversed(_undo):
                                    try:
                                        if _op == "update":
                                            _db.update(_tbl, _flt, _data)
                                        else:
                                            _db.insert(_tbl, [_data])
                                        _n_undo += 1
                                    except Exception:
                                        _n_fail += 1
                                st.error(f"병합 실패: {e} — 되돌림 {_n_undo}단계"
                                         + (f", 되돌리기 실패 {_n_fail}단계 "
                                            "(정합 점검 확인 필요)" if _n_fail
                                            else " (원래 상태로 복구)"))
    ...
    # [대체] / [별건 유지] 는 사람이 고른다. 품번당 카드 하나 — 옛 수주가
    # 여러 건으로 나뉘어 있어도 한 번에 처리 (2026-09-10 20/40/80/150AHYBV
    # 케이스). 결정은 so_line_replacements 에 남아 다시 묻지 않으며, 이력에서
    # 되돌릴 수 있다.
    from utils.so_replace import (detect_groups as _rp_detect,
    ...
    try:
        ...
        _rp_hist = fetch("so_line_replacements",
                         ...
                         "order=repl_id.desc", limit=300)
        _rp_decided = {(r["old_soi_id"], r["new_soi_id"]) for r in _rp_hist
                       if not r.get("reverted_at")}
        _rp_groups = _rp_detect(_rp_lines, _rp_decided)
    except Exception as e:
        st.warning(f"단가 변동 감지 실패: {e}")

    def _rp_apply(olds, n, action, upd_master, move_rounds, tag_side="new",
                  tag_kind="PROJECT"):
        """olds: 옛 라인 목록(체크한 것) → 새 라인 n. 쌍마다 이력 1건."""
        ...
        try:
            if action == "KEEP":
                ...
                _db.insert("so_line_replacements", [{
                    ...
                    "action": "KEEP", "old_price": o.get("unit_price"),
                    "new_price": n.get("unit_price"), "created_by": _who,
                    "old_so_status": (_ck_so.get(o["so_id"]) or {}).get("status"),
                    "old_line_status": o.get("status"),
                    "closed_pending_qty": 0, "closed_rounds": [],
                    "moved_rounds": [], "master_updated": False,
                    "master_old_price": None, "reason": None,
                    "tagged_soi_id": (n["soi_id"] if tag_side == "new"
                                      else o["soi_id"]),
                    "tagged_kind": tag_kind} for o in olds])
                st.session_state["rp_flash"] = (
                    "{} — 별건 유지: {} 수주를 {} 단가로 표시, 모든 수주 계속 진행 "
                    "(마스터 단가 변경 없음)".format(
            ...
            for o in olds:
                ...
                _recs.append({
                    "old_soi_id": o["soi_id"], "new_soi_id": n["soi_id"],
                    "action": "REPLACE", "old_price": o.get("unit_price"),
                    "new_price": n.get("unit_price"), "created_by": _who,
                    "old_so_status": (_ck_so.get(o["so_id"]) or {}).get("status"),
                    "old_line_status": o.get("status"),
                    "closed_pending_qty": pend,
                    "closed_rounds": [{"sched_id": s, "qty_before": q,
                                       "delivered": d} for s, q, d in close],
                    "moved_rounds": [], "master_updated": False,
                    "master_old_price": None, "reason": None,
                    "tagged_soi_id": None, "tagged_kind": None})
            _moved = []
            if _create_all:
                _tag = f"구 수주 회차 이관 {_today}"
                _db.insert("so_delivery_schedule", [{
                    "so_id": n["so_id"], "soi_id": n["soi_id"], "seq": i + 1,
                    "due_date": c["due_date"], "qty": c["qty"],
                    "delivered_qty": 0, "note": f"{_tag} ({c['src']})",
                    "created_by": _who}
                    for i, c in enumerate(sorted(_create_all,
            ...
            _db.update("sales_order_items", f"soi_id=eq.{_fq(n['soi_id'])}", {
                ...
                "remark": ((n.get("remark") or "") + " · " if n.get("remark")
                    ...
                    ", ".join(o["so_number"] for o in olds))})
            _m_upd, _m_old = False, None
            if upd_master and n.get("product_id"):
                ...
                if _new_sp > 0 and _db.update("products",
                    ...
                    _so_log("products", n["product_id"], "sale_price", _m_old,
                            ...
                            .format(n["so_number"],
                                    ", ".join(o["so_number"] for o in olds),
                                    float(olds[0].get("unit_price") or 0),
                                    _new_sp))
                    _m_upd = True
            if _recs:
                _recs[0]["moved_rounds"] = _moved
                for r in _recs:
                    r["master_updated"] = _m_upd
                    r["master_old_price"] = _m_old
                _db.insert("so_line_replacements", _recs)
            st.session_state["rp_flash"] = (
                "{} — 대체 처리: 옛 수주 {}건 미납 {:,.0f} 종료 · 회차 {}건 닫음 · "
                "새 회차 {}건{}".format(
                    n.get("pn"), len(olds), _closed_sum, _n_close,
                    len(_create_all), " · 마스터 단가 갱신" if _m_upd else ""))
            st.rerun()
        except Exception as e:
            st.error(f"처리 실패: {e}")
        ...
        with st.expander(
            ...
            for _gi, _g in enumerate(_rp_groups):
                ...
                with st.container(border=True):
                    st.markdown(
                        "**{}** · {} — 새 수주 **{}** ({}) · 수량 {:,.0f} · 단가 "
                        "{:,.0f}원 ({:+.1f}%) · 마스터 판매 단가 {}".format(
                            _n.get("pn") or "-", _n.get("customer"),
                            _n["so_number"], _n.get("so_date") or "-",
                            float(_n.get("qty") or 0), _np,
                            _g["diff_pct"] if _g["diff_pct"] is not None else 0,
                            f"{_msp:,.0f}원" if _msp else "미설정"))
                    _odf = pd.DataFrame([{
                        "포함": True, "옛 수주": o["so_number"],
                        "수주일": str(o.get("so_date") or "-")[:10],
                        "수량": float(o.get("qty") or 0),
                        "미납": float(o.get("pending_qty") or 0),
                        "단가": float(o.get("unit_price") or 0),
                        "납기": str(o.get("due_date") or "-")[:10],
                    } for o in _g["olds"]])
                    _oed = st.data_editor(
                        _odf, hide_index=True, use_container_width=True,
                        height=46 + 36 * len(_odf), key=_k + "_olds",
                        column_config={
                            "포함": st.column_config.CheckboxColumn(
                                help="대체 처리에 포함할 옛 라인"),
                            **{c: st.column_config.Column(disabled=True)
                               for c in ("옛 수주", "수주일", "수량", "미납",
                                         "단가", "납기")}})
                    _sel_olds = [_g["olds"][int(i)] for i, r in _oed.iterrows()
                                 if bool(r.get("포함"))]
        ...
        if mode == "파일 업로드 자동 파싱":
            ...
            if uploaded:
                ...
                if items:
                    ...
                    _mk = _so_mk
                    for p in products:
                        # 4번째 = 접두사 통합으로 휴면된 품번(별칭 취급) —
                        # 정확 일치라도 활성 본품으로 넘긴다 (2026-09-10:
                        # '4S20AHYBV-03-X1413;PM' 별칭이 휴면 P0010 에 걸림)
                        _v = (p['pn'], p['product_id'],
                              bool(p.get('archived_at')),
                              bool(p.get('archived_at'))
                              and "통합" in str(p.get("archive_reason") or ""))
                    ...
                    for it in items:
                        ...
                        for _ci, c in enumerate(candidates):
                            ...
                            if not c: continue
                            m = cm.get(_mk(c))
                            # 접두사 통합으로 휴면된 품번에 정확히 닿으면
                            # 그 품번은 별칭일 뿐 — 접두어를 뗀 활성 본품을
                            # 계속 찾는다 (없으면 마지막에 이 휴면으로)
                            if m and len(m) > 3 and m[3]:
                                _merged_hit = _merged_hit or m
                                m = None
                                continue
    ...
    with tab_ship:
        ...
        _cff1, _cff2 = st.columns([2, 1.4])
        _cf_filter = _cff1.radio("전표 상태", ["작성중", "확정", "취소", "전체"],
                                 horizontal=True, key="cf_filter",
                                 label_visibility="collapsed")
        # 전표 검색 (2026-09-16 리뷰: 옛 전표를 찾을 수단이 없던 문제) —
        # 전표번호·거래처·품번·작성자, 검색어가 있으면 최근 30건 제한 없이
        _cf_search = _cff2.text_input(
            "전표 검색", key="cf_search", label_visibility="collapsed",
            placeholder="검색 — 전표번호 · 거래처 · 품번 · 작성자")
        _cf_q = {"작성중": "status=eq.DRAFT", "확정": "status=eq.CONFIRMED",
                 "취소": "status=eq.CANCELLED",
        ...
        try:
            ...
            if _cf_sq:
                ...
                _cf_ships = fetch("shipments", _cf_cols,
                    ...
                    + (",shipment_id.in.(" + ",".join(
                       ...
                       if _cf_hit else "")
                    + ")&order=shipment_id.desc", limit=100)
            else:
                _cf_ships = fetch("shipments", _cf_cols,
            ...
            _cf_ships = []
        if not _cf_ships:
            st.info("검색 결과가 없습니다." if (_cf_search or "").strip()
                    else "전표가 없습니다 — 출고 등록 탭에서 리스트를 담아 "
                         "등록하세요.")
        else:
            ...
            _cf_ym = str(_cf_pick.get("ship_date") or "")[:7]
            _cf_locked = _cf_ym in _cf_closed
            try:
            ...
            _cf_names, _cf_lotmap = _cf_nl(
                fetch, _cf_items,
                confirmed=(_cf_pick.get("status") in ("CONFIRMED",
                                                      "CANCELLED")))

            def _cf_batch(items):
                # 정정으로 0 이 된 라인은 문서에서 제외 (이력에는 남음)
                items = [x for x in items if float(x.get("qty") or 0) > 0]
            ...
            def _cf_apply_deltas(pick, items, new_qtys, reason,
                                 ...
                                 keep_line_qty=False):
                """returns (처리 라인 수, 경고 목록). 경고가 있어도 처리된
                라인은 반영된 상태 — 사전 검증 실패는 (0, [오류]) 로 중단."""
                from datetime import datetime as _rv_now, date as _rv_dt
                from utils.shipment_adjust import (
                    plan_line_deltas, plan_lot_restore,
                    plan_round_release, split_round_alloc, line_status)
                _deltas = plan_line_deltas(items, new_qtys)
                if not _deltas:
                    return 0, ["변경된 수량이 없습니다."]
                _ship_no = pick["ship_no"]
                _sdate = str(pick.get("ship_date"))
                _today = _rv_dt.today().isoformat()
                _user = current_user_name()
                _rev = int(pick.get("rev_no") or 0) + 1
                _si_ids = [x["si_id"] for x, _, _, _ in _deltas]
                _si_str = ",".join(str(s) for s in _si_ids)
                _soi_ids = sorted({x["soi_id"] for x, _, _, _ in _deltas
                                   if x.get("soi_id") is not None})
                _txn_by, _alloc_by, _li_by = {}, {}, {}
                try:
                    for t in fetch("inventory_transactions",
                                   "txn_id,ref_id,lot_number,qty",
                                   "ref_table=eq.shipment_items"
                                   "&txn_type=eq.ISSUE"
                                   f"&ref_id=in.({_si_str})", limit=1000):
                        _txn_by.setdefault(t["ref_id"], []).append(t)
                    for a in fetch("shipment_allocations",
                                   "alloc_id,si_id,sched_id,qty",
                                   f"si_id=in.({_si_str})", limit=1000):
                        _alloc_by.setdefault(a["si_id"], []).append(a)
                    if _soi_ids:
                        _li_by = {l["soi_id"]: l for l in fetch(
                            "sales_order_items",
                            "soi_id,so_id,qty,received_qty,pending_qty,"
                            "unit", "soi_id=in.({})".format(
                                ",".join(str(s) for s in _soi_ids)),
                            limit=200)}
                except Exception as e:
                    return 0, [f"정정 준비 조회 실패: {e}"]
                # 증가분 사전 검증 — 수주 미납 한도 · 완성 재고
                _inc = [(x, d) for x, _, _, d in _deltas if d > 0]
                _stock9, _lots9, _used9 = {}, {}, {}
                _pids9 = {x.get("product_id") for x, _ in _inc
                          if x.get("product_id")}
                if _pids9:
                    _pstr9 = ",".join(f'"{p}"' for p in _pids9)
                    try:
                        _stock9 = {s["product_id"]:
                        ...
                        pass
                _errs = []
                _need_pend, _need_stk = {}, {}
                for x, d in _inc:
                    if x.get("soi_id") is not None:
                        _need_pend[x["soi_id"]] = (
                            _need_pend.get(x["soi_id"], 0) + d)
                    if x.get("product_id"):
                        _need_stk[x["product_id"]] = (
                            _need_stk.get(x["product_id"], 0) + d)
                for _soi9, _n9 in _need_pend.items():
                    _l9 = _li_by.get(_soi9)
                    if _l9 and _n9 > float(_l9.get("pending_qty") or 0) \
                            + 1e-9:
                        _errs.append(
                            "수주 미납 한도 초과 — 증가 {:,.0f} > 미납 "
                            "{:,.0f}".format(
                                _n9, float(_l9.get("pending_qty") or 0)))
                if not allow_over:
                    for _p9, _n9 in _need_stk.items():
                        if _n9 > _stock9.get(_p9, 0) + 1e-9:
                            _errs.append(
                                "완성 재고 부족 — {} 증가 {:,.0f} > 재고 "
                                "{:,.0f}".format(
                                    _p9, _n9, _stock9.get(_p9, 0)))
                if _errs:
                    return 0, _errs

                _warns, _txns9, _revs9, _n_ok = [], [], [], 0
                _so_ids9 = set()
                for x, old, new, d in _deltas:
                    _si = x["si_id"]
                    _soi = x.get("soi_id")
                    _li = _li_by.get(_soi) if _soi is not None else None
                    _pid = x.get("product_id")
                    _unit = (_li or {}).get("unit") or x.get("unit") or "EA"
                    _tag = "{} {}: {} ({})".format(
                        "출고 취소" if action == "CANCEL" else "출고 정정",
                        _ship_no, x.get("so_number") or "-",
                        reason or "-")
                    if d < 0:
                        _q = -d
                        # ① LOT 복원 — 마지막 차감 LOT 부터, 양수 ISSUE
                        _plan, _rest = plan_lot_restore(
                            _txn_by.get(_si, []), _q)
                        for _lot, _rq in _plan:
                            _txns9.append({
                                "material_id": None, "product_id": _pid,
                                "txn_type": "ISSUE", "qty": _rq,
                                "unit": _unit, "lot_number": _lot,
                                "work_order": _lot,
                                "ref_table": "shipment_items",
                                "ref_id": _si, "txn_date": _today,
                                "remark": _tag, "created_by": _user})
                        if _rest > 1e-9:
                            _warns.append(
                                "{} — 원장에 차감 기록이 없는 {:,.0f}개는 "
                                "재고 복원 없이 수주만 되돌렸습니다.".format(
                                    x.get("pn"), _rest))
                        # ② 회차 충당 되돌림
                        if _li:
                            try:
                                ...
                                if _rest9 > 1e-9:
                                    _warns.append(
                                        "{} — 회차 납품완료에서 {:,.0f}개를 "
                                        "되돌리지 못했습니다 (납품 스케줄 "
                                        "확인).".format(x.get("pn"), _rest9))
                            except Exception as e:
                                _warns.append(f"{x.get('pn')} 회차 되돌림 "
                                              f"실패: {e}")
                            # ③ 수주 라인
                            _nr = max(float(_li.get("received_qty") or 0)
                                      - _q, 0.0)
                            ...
                            _li["received_qty"] = _nr
                            _li["pending_qty"] = max(_nq - _nr, 0)
                    else:
                        # 증가: 수주 반영 → 회차 충당(지정 회차 우선) →
                        # LOT FIFO 차감 — 확정과 같은 규칙
                        if _li:
                            ...
                            try:
                                ...
                                _prev9 = {r["sched_id"]:
                                          float(r.get("delivered_qty") or 0)
                                          for r in _rr9}
                                _rr2 = [dict(r) for r in _rr9]
                                _al9, _left9 = {}, d
                                _own = next((r for r in _rr2 if r["sched_id"]
                                             == x.get("sched_id")), None)
                                if _own:
                                    _room = (float(_own.get("qty") or 0)
                                             - _prev9[_own["sched_id"]])
                                    _tk = min(max(_room, 0), _left9)
                                    if _tk > 0:
                                        _own["delivered_qty"] = (
                                            _prev9[_own["sched_id"]] + _tk)
                                        _al9[_own["sched_id"]] = \
                                            _own["delivered_qty"]
                                        _left9 -= _tk
                                if _left9 > 1e-9:
                                    _al9.update(_cf_alloc(_rr2, _left9,
                                                          _sdate))
                                ...
                                _rows9 = [{"si_id": _si, "sched_id": s,
                                           "qty": q}
                                          for _, s, q in split_round_alloc(
                                              [(_si, d, x.get("sched_id"))],
                                              _dl9)]
                                if _rows9:
                                    _db.insert("shipment_allocations",
                                               _rows9)
                            except Exception as e:
                                _warns.append(f"{x.get('pn')} 회차 충당 "
                                              f"실패: {e}")
                        if _pid:
                            _left5 = d
                            for _lot5 in _lots9.get(_pid, []):
                                if _left5 <= 1e-9:
                                    break
                                _k5 = (_pid, _lot5["lot_number"])
                                _lr = (float(_lot5.get("remain_qty") or 0)
                                       - _used9.get(_k5, 0))
                                _tk = min(_left5, max(_lr, 0))
                                if _tk <= 0:
                                    continue
                                _used9[_k5] = _used9.get(_k5, 0) + _tk
                                _txns9.append({
                                    "material_id": None, "product_id": _pid,
                    ...
                    _revs9.append({
                        ...
                        "action": action, "si_id": _si, "pn": x.get("pn"),
                        "qty_before": old, "qty_after": new,
                        "reason": reason or None, "created_by": _user})
                    if x.get("so_id"):
                        _so_ids9.add(x["so_id"])
                    _n_ok += 1
                if _txns9:
                    try:
                        _db.insert("inventory_transactions", _txns9)
                    except Exception as e:
                        _warns.append(f"재고 원장 기록 실패: {e}")
                try:
                    _db.insert("shipment_revisions", _revs9)
                except Exception as e:
                    _warns.append(f"정정 이력 기록 실패: {e}")
                for _sid7 in _so_ids9:
                    try:
                        ...
                        pass
                _hdr = {"rev_no": _rev,
                        "revised_at": _rv_now.now().isoformat()}
                if action == "CANCEL":
                    _hdr.update({"status": "CANCELLED",
                                 "cancelled_at": _rv_now.now().isoformat(),
            ...
            def _cf_rev_history(pick):
                try:
                    ...
                    _rvs = []
                if _rvs:
                    with st.expander("정정 이력 {}건".format(len(_rvs))):
                        toss_table([{
                            "회차": f"v{int(r.get('rev_no') or 0) + 1}",
                            "구분": ("취소" if r.get("action") == "CANCEL"
                                   else "정정"),
                            "품번": r.get("pn") or "-",
                            "이전": float(r.get("qty_before") or 0),
                            "이후": float(r.get("qty_after") or 0),
                            "사유": r.get("reason") or "-",
                            "담당": r.get("created_by") or "-",
                            "일시": str(r.get("created_at") or "")[:16],
                        } for r in _rvs], num_cols=("이전", "이후"),
                            strong_cols=("품번",))
            ...
            if not _cf_items:
                ...
                toss_df(_cf_pd.DataFrame([{
                    ...
                    column_config={c: st.column_config.NumberColumn(
                        format="localized")
                        for c in ("수량", "단가")})
                st.caption("취소 {} · 사유 {}{}".format(
                    str(_cf_pick.get("cancelled_at") or "")[:16] or "-",
                    _cf_pick.get("cancel_reason") or "-",
                    " · 확정 후 취소 (수주·재고 역반영됨)"
                    if _cf_pick.get("confirmed_at") else " · 작성중 취소"))
                _cf_rev_history(_cf_pick)
            elif _cf_pick.get("status") == "CONFIRMED":
                import pandas as _cf_pd
                from utils.shipment_adjust import rev_label as _cf_rl
                _cf_rev_lbl = _cf_rl(_cf_pick.get("rev_no"),
                                     _cf_pick.get("revised_at"))
                toss_df(_cf_pd.DataFrame([{
                    "품번": x.get("pn"),
                    "품명": _cf_names.get(x.get("si_id")) or "-",
                ...
                st.caption("확정 {} · 처리 {}{}".format(
                    ...
                    f" · {_cf_rev_lbl} (재발행 문서에 표기)"
                    if _cf_rev_lbl else ""))
                pr1, pr2 = st.columns(2)
                pr1.download_button(
                    "출고 리스트 재발행",
                    _cf_list(_cf_batch(_cf_items), rev_label=_cf_rev_lbl),
                ...
                if _cf_locked:
                    ...
                    with st.expander("전표 정정 (수량 변경 · 라인 제외)"):
                        _rv_df = _cf_pd.DataFrame([{
                            ...
                            "품명": _cf_names.get(x.get("si_id")) or "-",
                            "거래처": x.get("customer"),
                            "현재": float(x.get("qty") or 0),
                            "정정 수량": float(x.get("qty") or 0),
                        } for x in _cf_items])
                        _rv_ed = st.data_editor(
                            _rv_df, hide_index=True,
                            use_container_width=True,
                            key="cf_rv_ed_{}_{}".format(
                                _cf_pick["shipment_id"],
                                _cf_pick.get("rev_no") or 0),
                            column_config={
                                "정정 수량": st.column_config.NumberColumn(
                                    "정정 수량", min_value=0, step=1),
                                **{c: st.column_config.Column(
                                    disabled=True)
                                   for c in ("품번", "품명", "거래처",
                                             "현재")},
                            })
                        _rv_reason = st.text_input(
                            "정정 사유 (필수)", key="cf_rv_reason_{}".format(
                                _cf_pick["shipment_id"]),
                            placeholder="예: 현장 확인 수량 착오, 라인 중복")
                        _rv_over = st.checkbox(
                            "재고 없이 증가 허용", value=False,
                            key="cf_rv_over",
                            help="수량을 늘릴 때 완성 재고와 무관하게 차감")
                        _rv_new = {}
                        for _bi, _brow in _rv_ed.iterrows():
                            _x = _cf_items[int(_bi)]
                            _rv_new[_x["si_id"]] = float(_cf_pd.to_numeric(
                                _brow.get("정정 수량"), errors="coerce")
                                or 0)
                        _rv_changed = [
                            (x, _rv_new[x["si_id"]]) for x in _cf_items
                            if abs(_rv_new[x["si_id"]]
                                   - float(x.get("qty") or 0)) > 1e-9]
                        if _rv_changed:
                            st.caption("변경 {}건 — ".format(len(_rv_changed))
                                       + ", ".join(
                                "{} {:,.0f}→{:,.0f}".format(
                        ...
                        if st.button("정정 적용", key="cf_rv_go",
                            ...
                            st.session_state["cf_rv_plan"] = (
                                _rv_new, _rv_reason.strip(), _rv_over)
                        if st.session_state.get("cfm_cf_rv") \
                                and confirm_gate("cf_rv",
                                    "전표 {} 를 정정합니다 — 변경 {}건의 "
                                    "차이만큼 수주 납품·회차·재고를 "
                                    "되돌리거나 추가 차감합니다. "
                                    "실행할까요?".format(
                                        _cf_pick["ship_no"],
                                        len(_rv_changed))) \
                                and click_guard("cf_rv"):
                            _pl = st.session_state.pop("cf_rv_plan", None) \
                                or (_rv_new, _rv_reason.strip(), _rv_over)
                            _n_ok, _ws = _cf_apply_deltas(
                                _cf_pick, _cf_items, _pl[0], _pl[1],
                                action="EDIT", allow_over=_pl[2])
                            for _w in _ws:
                    ...
                    if st.session_state.get("cfm_cf_cancel") \
                            and confirm_gate("cf_cancel",
                                "확정된 전표 {}를 취소합니다 — 전 라인의 "
                                "수주 납품·회차 충당·완성 재고 차감을 "
                                "되돌립니다. 전표번호와 라인은 기록으로 "
                                "남습니다. 실행할까요?".format(
                                    _cf_pick.get("ship_no") or "")) \
                            and click_guard("cf_cancel"):
                        _n_ok, _ws = _cf_apply_deltas(
                            _cf_pick, _cf_items,
                            {x["si_id"]: 0.0 for x in _cf_items},
                            _cx_reason.strip(), action="CANCEL",
                            keep_line_qty=True)
                        for _w in _ws:
                            (st.warning if _n_ok else st.error)(_w)
                        if _n_ok:
                ...
                if st.button(
                    ...
                    for _soi5, _e in _by_soi.items():
                        ...
                        try:
                            ...
                            _dl6 = {s: v - _prev6.get(s, 0)
                                    for s, v in _al6.items()}
                            _ln6 = [(x["si_id"], q, x.get("sched_id"))
                                    for x, q in _cf_rows
                                    if x["soi_id"] == _soi5]
                            _ar6 = [{"si_id": a, "sched_id": b, "qty": c}
                                    for a, b, c in _cf_split(_ln6, _dl6)]
                            if _ar6:
                                _db.insert("shipment_allocations", _ar6)
                ...
                if st.session_state.get("cfm_cf_cancel") and confirm_gate(
                    ...
                    try:
                        ...
                        _db.update("shipments",
                                   ...
                                   {"status": "CANCELLED",
                                    "cancelled_at":
                                    _cx_now.now().isoformat(),
                                    "cancel_reason": "작성중 취소"})
                        st.success("전표 취소됨")
                        st.rerun()
                    except Exception as e:
                        st.error(f"취소 실패: {e}")
    ...
    with tab_fstock:
        ...
        # 완성 재고 = 실제 완성(검사 판정)으로 기록된 LOT 잔여의 합.
        # 장부 합계(product_stock_v — 기초 조정·마이너스 포함)는 참고 컬럼으로만
        # (2026-09-16 사용자: 실사 전 마이너스 수량은 참고치로 두고 실제 완성만
        # 재고로 본다). LOT 만 있고 장부에 없는 품목도 포함.
        _fs_rows = []
        _fs_seen = set()
        _fs_all = list(_fs)
        for _pid9, _ls9 in _fs_lots.items():
            if not any(r["product_id"] == _pid9 for r in _fs):
                _fs_all.append({"product_id": _pid9, "pn": _ls9[0].get("pn"),
                                "customer": None, "current_stock": 0,
                                "last_txn_date": None})
        for r in _fs_all:
            if r["product_id"] in _fs_seen:
                continue
            _fs_seen.add(r["product_id"])
            _lots9 = _fs_lots.get(r["product_id"], [])
            _stk = sum(_fsf(l.get("remain_qty")) for l in _lots9)
            _book = _fsf(r.get("current_stock"))
            _pd9 = _fs_pend.get(r["product_id"], 0.0)
            if _stk <= 0 and _book == 0 and _pd9 <= 0:
                continue
            _fs_rows.append({
                "품번": r.get("pn") or r["product_id"],
                "거래처": r.get("customer") or "-",
                "완성 재고": _stk, "미납": _pd9, "여유": _stk - _pd9,
                "LOT": float(len(_lots9)),
                "장부 (참고)": _book,
        ...
        _fk3.metric("미납 대비 부족 품목",
                    ...
                    help="완성 재고(실제 완성 LOT 잔여 합) − 미납 < 0 인 품목. "
                         "'장부 (참고)' 는 기초 조정·마이너스를 포함한 장부 합계로, "
                         "실사 전에는 참고만")
        if not _fs_rows:
            ...
            st.markdown(f"##### {_fr['품번']} — 완성 재고 {_fr['완성 재고']:,.0f} · "
                        ...
                        + (f" · 장부 {_fr['장부 (참고)']:,.0f} (참고)"
                           if abs(_fr['장부 (참고)'] - _fr['완성 재고']) > 0.5
                           else ""))
            if _fl:
                toss_table([{
    ...
    # 표준 = 공정 설계 단계에서 정하는 작업 표준(공정 개선 때만 바뀜).
    # 시트 UPH 목표·실적 중앙값은 참고 컬럼. 저장한 행은 CONFIRMED 로 잠기고
    # 재산출은 자동값만 갱신. 변경은 product_op_std_log 에 남는다.
    with tab_std:
        ...
        if sc1.button("실적에서 자동값 재산출", key="std_recalc",
                      use_container_width=True,
                      help="실적 중앙값·설비 매트릭스만 갱신. 확정 표준은 "
                           "건드리지 않음") \
            ...
            try:
                ...
                if _newm:
                    _db.insert("machines", [{
                        ...
                        for m in _newm])
                st.success("재산출 — 신규 {} · 갱신 {} · 설비 매트릭스 {}건 · "
                           "새 설비 {}대 (실적 {}행)".format(
        ...
        try:
            _stds = _std_fetch_all(
                ...
                "uph_auto,uph_mean,uph_sheet,ct_sec,sample_n,hours_n,"
                "machines_used,setup_min,std_status,std_source,active,"
                "calc_at,first_date,last_date,confirmed_by,confirmed_at,note",
                "pn,step,process")
        except Exception as e:
            st.error(f"표준 조회 실패: {e}")
            _stds = []
        _need_pids = {i.get("product_id") for i in sois if i.get("product_id")}
        _need_pns = {i.get("canonical_pn") for i in sois
                     if i.get("canonical_pn")}
        _std_pids = {s["product_id"] for s in _stds if s.get("product_id")
                     and s.get("active", True)}
        _n_conf = sum(1 for s in _stds if s.get("std_status") == "CONFIRMED"
                      and s.get("active", True))
        _n_act = sum(1 for s in _stds if s.get("active", True))
        sc2.markdown(
            '<div class="kpi-row">'
            '<div class="kpi"><div class="k">작업 표준</div>'
            f'<div class="v">{_n_act}</div>'
            f'<div class="s">확정 {_n_conf} · 자동 {_n_act - _n_conf} · '
            f'제외 {len(_stds) - _n_act}</div></div>'
            '<div class="kpi {c}"><div class="k">미납 품번 커버리지</div>'
            '<div class="v">{a}/{b}</div><div class="s">표준 있는 품번 / '
            '미납 품번</div></div></div>'.format(
                a=len(_need_pids & _std_pids), b=len(_need_pids),
                c="good" if _need_pids and _need_pids <= _std_pids
                else "warn"),
            unsafe_allow_html=True)
        _miss_pns = sorted({i.get("canonical_pn") for i in sois
                            if i.get("product_id")
                            and i["product_id"] not in _std_pids
                            and i.get("canonical_pn")})
        if _miss_pns:
            with st.expander(f"표준 없는 미납 품번 {len(_miss_pns)}종 "
                             "(아래에서 품번을 새로 등록하거나 시트 품번을 "
                             "제품에 연결)"):
        ...
        _by_pn = {}
        for s in _stds:
            _by_pn.setdefault(s["pn"], []).append(s)
        _pid_nm = {}
        try:
            _pids_s = sorted({s["product_id"] for s in _stds
                              if s.get("product_id")})
            for _i0 in range(0, len(_pids_s), 80):
                _pid_nm.update({p["product_id"]: p for p in fetch(
                    "products", "product_id,pn,item_name,archived_at",
                    "product_id=in.({})".format(",".join(
                        f'"{x}"' for x in _pids_s[_i0:_i0 + 80])),
                    limit=300)})
        except Exception:
            pass
        _pn_rows = []
        for pn, ops in _by_pn.items():
            _act = [o for o in ops if o.get("active", True)]
            _cf = [o for o in _act if o.get("std_status") == "CONFIRMED"]
            _pid = next((o["product_id"] for o in ops if o.get("product_id")),
                        None)
            _pm = _pid_nm.get(_pid) or {}
            _pn_rows.append({
                "품번": pn,
                "제품": (_pm.get("pn") or "미연결") + (
                    " (휴면)" if _pm.get("archived_at") else ""),
                "품명": (_pm.get("item_name") or "-")[:28],
                "공정": len(_act),
                "확정": len(_cf),
                "표준 UPH": " / ".join(
                    f"{float(o.get('uph_std') or 0):g}" for o in sorted(
                        _act, key=lambda o: (o.get("step") or 0))),
                "미납": "미납" if _pid in _need_pids else "-",
                "상태": ("미연결" if not _pid else
                       "확정" if _act and len(_cf) == len(_act) else
                       "일부" if _cf else "자동"),
            })
        with st.form("std_filter_form"):
            sf1, sf2, sf3, sf4 = st.columns([2, 1, 1, 1])
            _sq = sf1.text_input("품번 · 품명 검색", key="std_q")
            _sst = sf2.selectbox("상태", ["전체", "자동", "일부", "확정",
                                        "미연결"], key="std_st")
            _spend = sf3.selectbox("범위", ["전체", "미납만"], key="std_pend")
            sf4.form_submit_button("검색", use_container_width=True)
        _sv = [r for r in _pn_rows
               if (not _sq.strip() or _sq.strip().lower() in
                   (r["품번"] + " " + r["제품"] + " " + r["품명"]).lower())
               and (_sst == "전체" or r["상태"] == _sst)
               and (_spend == "전체" or r["미납"] == "미납")]
        _sv.sort(key=lambda r: (r["미납"] != "미납", r["품번"]))

        # ── 새 품번 표준 등록 ──
        with st.expander("새 품번의 작업 표준 등록"):
            with st.form("std_new_form"):
                nf1, nf2, nf3, nf4 = st.columns([2, 1, 1, 1])
                _n_pn = nf1.text_input("품번 (시트 표기, 예: 8HFDV-05)",
                                       key="std_new_pn")
                _n_steps = nf2.text_input("스텝 (콤마)", "10,20",
                                          key="std_new_steps")
                _n_grp = nf3.selectbox("장비군", ["CNC", "MCT", "ETC"],
                                       key="std_new_grp")
                _n_uph = nf4.number_input("표준 UPH", 0.0, 100000.0, 20.0,
                                          1.0, key="std_new_uph")
                if st.form_submit_button("등록", type="primary"):
                    _pn_n = _n_pn.strip().upper()
                    if not _pn_n:
                        st.error("품번을 입력하세요.")
                    else:
                        from utils.prod_sheet import (
                            build_pn_index as _bpi, pn_key as _pk)
                        try:
                            _pidx = _bpi(_std_fetch_all(
                                "products", "product_id,pn,alias_list,"
                                "archived_at", "product_id"))
                            _pid_n = _pidx.get(_pk(_pn_n))
                            _rows_n = []
                            for _stp in [x.strip() for x in
                                         _n_steps.split(",") if x.strip()]:
                                _proc = f"{_pn_n}#{_stp}"
                                if (_pn_n, _proc) in {(s["pn"], s["process"])
                                                      for s in _stds}:
                                    continue
                                _rows_n.append({
                                    "pn": _pn_n, "process": _proc,
                                    "step": (int(_stp) if _stp.isdigit()
                                             else None),
                                    "product_id": _pid_n,
                                    "group_code": _n_grp,
                                    "uph_std": _n_uph, "setup_min": 0,
                                    "std_status": "CONFIRMED",
                                    "std_source": "MANUAL", "active": True,
                                    "confirmed_by": current_user_name(),
                                    "confirmed_at":
                                    _std_now.now().isoformat()})
                            if _rows_n:
                                _db.insert("product_op_std", _rows_n)
                                st.success("{} 작업 {}건 등록{}".format(
                                    _pn_n, len(_rows_n),
                                    "" if _pid_n else " — 제품 미연결 "
                                    "(아래 상세에서 연결)"))
                                st.rerun()
                            else:
                                st.info("이미 있는 작업입니다.")
        ...
        if not _sv:
            st.info("조건에 맞는 품번이 없습니다.")
        else:
            _si = toss_grid(_sv, key="std_grid",
                            badge_cols=("상태", "미납"), strong_cols=("품번",),
                            num_cols=("공정", "확정"),
                            height=520 if len(_sv) > 12 else None)
            _sp_pn = _sv[_si if _si is not None else 0]["품번"]
            _ops = sorted(_by_pn.get(_sp_pn, []),
                          key=lambda o: (not o.get("active", True),
                                         o.get("step") is None,
                                         o.get("step") or 0, o["process"]))
            _sp_pid = next((o["product_id"] for o in _ops
                            if o.get("product_id")), None)
            _sp_pm = _pid_nm.get(_sp_pid) or {}
            st.markdown("##### {} — {} {}".format(
                _sp_pn, _sp_pm.get("pn") or "제품 미연결",
                f"· {_sp_pm.get('item_name')}" if _sp_pm.get("item_name")
                else ""))

            # 제품 미연결 → 검색해 연결 (그 품번의 모든 작업에 적용)
            if not _sp_pid:
                lk1, lk2 = st.columns([3, 1])
                _lk_q = lk1.text_input("연결할 제품 품번 검색",
                                       value=_sp_pn.split("#")[0],
                                       key=f"std_link_q_{_sp_pn}")
                _lk_c = []
                if _lk_q.strip():
                    try:
                        ...
                        _lk_c = []
                if _lk_c:
                    _lk_pick = lk1.selectbox(
                        "후보", _lk_c, key=f"std_link_pick_{_sp_pn}",
                        format_func=lambda p: "{}{} · {}".format(
                            p["pn"], " (휴면)" if p.get("archived_at") else "",
                            p.get("item_name") or "-"))
                    if lk2.button("연결", key=f"std_link_go_{_sp_pn}",
                                  use_container_width=True):
                        try:
                            _db.update("product_op_std",
                                       f"pn=eq.{_fq(_sp_pn)}",
                    ...
                    lk1.caption("일치하는 제품이 없습니다 — 마스터 관리에서 "
                                "제품을 먼저 등록하세요.")

            # 작업(공정) 편집 표 — st.form + 편집 취소(nonce)
            _grps = sorted({m["group_code"] for m in fetch(
                "machines", "group_code", limit=500)} | {"ETC"})
            _std_df = pd.DataFrame([{
                "사용": bool(o.get("active", True)),
                "공정": o["process"],
                "스텝": int(o["step"]) if o.get("step") is not None else None,
                "장비군": o.get("group_code") or "ETC",
                "표준 UPH": float(o.get("uph_std") or 0),
                "준비(분)": float(o.get("setup_min") or 0),
                "메모": o.get("note") or "",
                "상태": ("확정" if o.get("std_status") == "CONFIRMED"
                       else "자동"),
                "시트 UPH": float(o.get("uph_sheet") or 0) or None,
                "실적 UPH": float(o.get("uph_auto") or 0) or None,
                "표본": int(o.get("sample_n") or 0),
                "설비": o.get("machines_used") or "",
            } for o in _ops])
            _std_nonce = st.session_state.setdefault("std_nonce", 0)
            with st.form("std_edit_form"):
                _std_ed = st.data_editor(
                    _std_df, hide_index=True, use_container_width=True,
                    key=f"std_ed_{_sp_pn}_{_std_nonce}",
                    column_config={
                        "사용": st.column_config.CheckboxColumn(
                            help="끄면 작업 큐·스케줄에서 제외 (변형 공정 정리)"),
                        "공정": st.column_config.TextColumn(
                            help="작업 이름 = 품번#스텝 (시트 표기와 같아야 "
                                 "실적이 붙습니다)"),
                        "스텝": st.column_config.NumberColumn(
                            min_value=0, max_value=999, step=10),
                        "장비군": st.column_config.SelectboxColumn(
                            options=_grps),
                        "표준 UPH": st.column_config.NumberColumn(
                            min_value=0.0, step=0.5, format="%.1f"),
                        "준비(분)": st.column_config.NumberColumn(
                            min_value=0.0, step=5.0),
                        **{c: st.column_config.Column(disabled=True)
                           for c in ("상태", "시트 UPH", "실적 UPH", "표본",
                                     "설비")},
                    })
                _std_reason = st.text_input(
                    "변경 사유 (선택 — 공정 개선·설계 변경 등)",
                    key=f"std_reason_{_sp_pn}")
                sb1, sb2 = st.columns(2)
                _std_save = sb1.form_submit_button("표준 저장 (확정)",
                                                   type="primary",
                                                   use_container_width=True)
            ...
            if _std_save:
                _n_chg, _logs9 = 0, []
                _now9 = _std_now.now().isoformat()
                _who = current_user_name()
                for _bi, _brow in _std_ed.iterrows():
                    if int(_bi) >= len(_ops):
                        continue
                    o = _ops[int(_bi)]
                    _new = {
                        "active": bool(_brow.get("사용")),
                        "process": str(_brow.get("공정") or "").strip()
                        or o["process"],
                        "step": (int(_brow["스텝"]) if pd.notna(
                            _brow.get("스텝")) and _brow.get("스텝")
                                 not in ("", None) else None),
                        "group_code": str(_brow.get("장비군") or "ETC"),
                        "uph_std": float(_brow.get("표준 UPH") or 0),
                        "setup_min": float(_brow.get("준비(분)") or 0),
                        "note": str(_brow.get("메모") or "").strip() or None,
                    }
                    _old = {
                        "active": bool(o.get("active", True)),
                        "process": o["process"],
                        "step": (int(o["step"]) if o.get("step") is not None
                                 else None),
                        "group_code": o.get("group_code") or "ETC",
                        "uph_std": float(o.get("uph_std") or 0),
                        "setup_min": float(o.get("setup_min") or 0),
                        "note": o.get("note") or None,
                    }
                    _diff = {k: v for k, v in _new.items() if v != _old[k]}
                    if not _diff:
                        continue
                    _diff.update({"std_status": "CONFIRMED",
                                  "confirmed_by": _who, "confirmed_at": _now9,
                                  "updated_at": _now9})
                    try:
                        if _db.update("product_op_std",
                            ...
                            for k, v in _diff.items():
                                if k in _old:
                                    _logs9.append({
                                        "op_id": o["op_id"], "pn": o["pn"],
                                        "process": o["process"], "field": k,
                                        "old_value": str(_old[k]),
                                        "new_value": str(v),
                                        "reason": _std_reason.strip() or None,
                                        "changed_by": _who})
                    except Exception as e:
                        st.error(f"{o['process']} 저장 실패: {e}")
                if _logs9:
                    try:
                        _db.insert("product_op_std_log", _logs9)
                    except Exception:
                        pass
                st.success(f"표준 저장 — {_n_chg}개 작업 확정")
                st.session_state["std_nonce"] = _std_nonce + 1
                st.rerun()

            # 설비별 실적 (능력 매트릭스) + 변경 이력
            try:
            ...
            with xm1:
                if _pm9:
                    with st.expander(f"설비별 실적 {len(_pm9)}건 (능력 매트릭스)"):
                        toss_table([{
                            "공정": m["process"], "설비": m["machine_id"],
                            "실적 UPH": float(m.get("uph_actual") or 0),
                            "구간": int(m.get("runs") or 0),
                            "시간": float(m.get("hours") or 0),
                            "최근": m.get("last_date") or "-",
                        } for m in _pm9], num_cols=("실적 UPH", "구간", "시간"),
                            strong_cols=("설비",))
            with xm2:
                if _lg9:
                    _fk9 = {"uph_std": "표준 UPH", "setup_min": "준비(분)",
                            "group_code": "장비군", "active": "사용",
                            "process": "공정", "step": "스텝", "note": "메모"}
                    with st.expander(f"변경 이력 {len(_lg9)}건"):
                        toss_table([{
                            "공정": l["process"],
                            "항목": _fk9.get(l["field"], l["field"]),
                            "이전": l.get("old_value") or "-",
                            "이후": l.get("new_value") or "-",
                            "사유": l.get("reason") or "-",
                            "담당": l.get("changed_by") or "-",
                            "일시": str(l.get("changed_at") or "")[:16],
                        } for l in _lg9], strong_cols=("공정",))

    # ─── 탭: 설비 스케줄 보드 (1단계 — 작업 큐 · 보드 · 셀 편집) ───
    # 목업 승인 2026-09-07. 셀 = 설비 × 날짜 × 교대, 작업 = 품번#스텝.
    # 2026-09-07 개편(사용자: 내용이 많아 쓰기 어렵다): 보드가 메인 —
    # [계획 입력] 모드는 시트처럼 셀에서 작업을 고르는 격자(data_editor,
    # 드롭다운 검색), 수량은 표준 UPH × 교대 시간으로 자동. [보드 보기] 는
    # 실적이 붙은 색 칩. 작업 큐·기간 일괄 입력·수량 조정은 접힌 영역.
    with tab_sb:
        ...
        wn1, wn2, wn3, wn4, wn5 = st.columns([1, 2.6, 1, 1, 1.6])
        if wn1.button("‹ 이전 주", key="sb_prev", use_container_width=True,
                      help="저장하지 않은 보드 변경은 주를 옮기면 사라집니다 — 먼저 [저장]"):
        ...
        try:
            ...
            for _i0 in range(0, len(_sb_pids), 80):
                ...
                _sb_stds += fetch("product_op_std",
                                  ...
                                  f"product_id=in.({_pstr})&active=eq.true"
                                  "&order=pn,step", limit=1000)
        ...
        _sb_act_sum = sum(c["actual"] for c in _sb_cells.values()
                          if c["plan"])
        # 작업 사전 (표준 있는 미납 작업 + 이번 주에 이미 잡힌 작업)
        _sb_ops = {}
        for q in _sb_queue:
            if not q["no_std"]:
                _sb_ops[(q["pn"], q["process"])] = q
        for s in _sb_sched:
            k = (s["pn"], s["process"])
            if k not in _sb_ops:
                _sb_ops[k] = {"pn": s["pn"], "process": s["process"],
                              "product_id": s.get("product_id"),
                              "due": None, "net": 0.0, "alloc": 0.0,
                              "uph": None, "machines": [], "no_std": True}
        _sb_std_by = {(s["pn"], s["process"]): s for s in _sb_stds}

        def _sb_lbl(k):
            q = _sb_ops[k]
            _left = max(q["net"] - q["alloc"], 0)
            return "{} · {}{}".format(
                q["process"],
                (q["due"][5:] if q["due"] else "납기없음"),
                f" · 남은 {_left:,.0f}" if q["net"] else "")

        _sb_labels = {_sb_lbl(k): k for k in sorted(
            _sb_ops, key=lambda k: (_sb_ops[k]["due"] or "9999", k))}
        _sb_k2lbl = {k: l for l, k in _sb_labels.items()}

        # 상태 한 줄
        _line = " · ".join(
            f"{g} 부하 {(p / a * 100) if a else 0:.0f}%"
            for g, (p, a) in sorted(_sb_gl.items()))
        wn5.markdown(
            "<div style='padding-top:8px;font-size:12.5px;color:#4e5968'>{}"
            "<br>미배정 {}건 · 납기 위험 <b style='color:{}'>{}</b>건 · "
            "계획 {:,.0f} / 실적 {:,.0f}</div>".format(
                _line, len(_sb_unassigned),
                "#f04452" if _sb_late else "#01a76b", len(_sb_late),
                _sb_plan_sum, _sb_act_sum), unsafe_allow_html=True)

        # ── 보드 컴포넌트 (components/schedule_board, 2026-09-09 현장 요청:
        # 캘린더에서 직접 넣고 빼고 한 번에) — 클릭 배치·드래그 이동/복사·
        # 채우기 핸들·×·되돌리기, [저장] 시 변경 목록만 앱으로 전달.
        # 프로토콜은 index.html 이 직접 구현(streamlit:componentReady /
        # render / setComponentValue). 컴포넌트가 없으면 아래 격자로 폴백.
        _sb_grp = st.radio("장비군", ["전체"] + sorted(
            {m.get("group_code") or "ETC" for m in _sb_act}),
            horizontal=True, key="sb_grp", label_visibility="collapsed")
        _sb_rows_m = [m for m in _sb_act
                      if _sb_grp == "전체"
                      or (m.get("group_code") or "ETC") == _sb_grp]
        _wd = "월화수목금토일"
        _sb_cols = [(d, sh) for d in _sb_days for sh in _sch.SHIFTS]
        _sb_colname = {(d, sh): "{}/{} {} {}".format(
            d.month, d.day, _wd[d.weekday()], "주" if sh == "주간" else "야")
            for d, sh in _sb_cols}
        _sb_use_comp = False
        try:
            import os as _sb_os
            import streamlit.components.v1 as _sb_cv1
            _sb_comp_dir = _sb_os.path.join(
                _sb_os.path.dirname(_sb_os.path.abspath(__file__)),
                "components", "schedule_board")
            if _sb_os.path.exists(_sb_os.path.join(_sb_comp_dir,
                                                   "index.html")):
                if "ws_schedule_board" not in globals():
                    globals()["ws_schedule_board"] = _sb_cv1.declare_component(
                        "ws_schedule_board", path=_sb_comp_dir)
                _sb_comp = globals()["ws_schedule_board"]
                _sb_use_comp = True
        except Exception:
            _sb_use_comp = False

        if _sb_use_comp:
            if st.session_state.get("sb_flash"):
                st.success(st.session_state.pop("sb_flash"))
            _sb_gen = st.session_state.get("sb_comp_gen", 0)
            _sb_nonce_c = "{}:{}:{}:{}".format(
                _sb_d0, len(_sb_sched),
                sum(int(s["sched_id"]) for s in _sb_sched), _sb_gen)
            _c_machines = [{
                "id": m["machine_id"], "name": m.get("name") or "",
                "group": m.get("group_code") or "ETC",
                "night": bool(m.get("night_active", True)),
                "hours": float(m.get("hours_per_shift") or 9)}
                for m in _sb_rows_m]
            _c_cells = {}
            for (mid, d, sh), c in _sb_cells.items():
                key = f"{mid}|{d}|{sh}"
                if c["plan"]:
                    _c_cells[key] = {
                        "plan": True, "pn": c["plan"]["pn"],
                        "process": c["plan"]["process"],
                        "qty": float(c["plan"].get("plan_qty") or 0),
                        "hours": float(c["plan"].get("plan_hours") or 9),
                        "status": c["status"], "actual": c["actual"]}
                elif c["status"] == "STOP":
                    _c_cells[key] = {"plan": False, "status": "STOP",
                                     "stop": c.get("stop")}
                else:
                    _c_cells[key] = {"plan": False, "status": "ACTUAL",
                                     "process": c.get("process"),
                                     "actual": c["actual"]}
            _c_ops = []
            for k2 in sorted(_sb_ops, key=lambda k: (
                    _sb_ops[k]["due"] or "9999", k)):
                q = _sb_ops[k2]
                _std9 = _sb_std_by.get(k2, {})
                _c_ops.append({
                    "key": f"{k2[0]}||{k2[1]}", "pn": k2[0],
                    "process": k2[1],
                    "uph": float(_std9.get("uph_std") or 0) or None,
                    "setup": float(_std9.get("setup_min") or 0),
                    "due": q.get("due"),
                    "left": (max(q["net"] - q["alloc"], 0) if q.get("net")
                             else None),
                    "machines": list(q.get("machines") or [])})
            _ret = _sb_comp(machines=_c_machines,
                            days=[d.isoformat() for d in _sb_days],
                            cells=_c_cells, ops=_c_ops, nonce=_sb_nonce_c,
            ...
            if isinstance(_ret, dict) and _ret.get("changes"):
                ...
                if (st.session_state.get("sb_comp_applied") != _tag
                    ...
                    _n_ins, _n_upd, _n_del, _bad = 0, 0, 0, []
                    _ins_rows = []
                    _now_s = _sb_now.now().isoformat()
                    for ch in _ret["changes"]:
                        mid, d, sh = (ch.get("machine_id"), ch.get("date"),
                                      ch.get("shift"))
                        m = next((x for x in _sb_act
                                  if x["machine_id"] == mid), None)
                        if not m or sh not in _sch.SHIFTS or not d:
                            continue
                        _old = _cur.get((mid, d, sh))
                        ...
                        if _k2 not in _sb_ops:
                            _bad.append(f"{mid} {d} {sh}: {ch.get('process')}")
                            continue
                        if sh == "야간" and not m.get("night_active", True):
                            continue
                        _std9 = _sb_std_by.get(_k2, {})
                        _hps = float(m.get("hours_per_shift") or 9)
                        _qty9 = _sch.suggest_qty(
                            _hps, _std9.get("uph_std"),
                            _std9.get("setup_min") or 0)
                        rec = {"pn": _k2[0], "process": _k2[1],
                               "product_id": _sb_ops[_k2].get("product_id"),
                               "plan_qty": float(_qty9), "plan_hours": _hps,
                               "updated_at": _now_s}
                        if _old:
                    ...
                    try:
                        ...
                        st.session_state["sb_flash"] = (
                            "보드 저장 — 신규 {} · 변경 {} · 비움 {}".format(
                                _n_ins, _n_upd, _n_del)
                            + (" · 인식 못한 작업: " + "; ".join(_bad[:8])
                               if _bad else ""))
                        st.session_state["sb_comp_gen"] = _sb_gen + 1
                        st.rerun()
                    except Exception as e:
                        st.error(f"저장 실패: {e}")
        ...
        if not _sb_use_comp:
            # ── 모드: 보드 보기 / 계획 입력 ──
            _has_plan = any(c["plan"] for c in _sb_cells.values())
            _sb_mode = st.radio(
                "보드 모드", ["계획 입력", "보드 보기"],
                index=0 if (not _has_plan and _sb_d1 >= _sb_today.isoformat())
                else 1, horizontal=True, key=f"sb_mode_{_sb_d0}",
                label_visibility="collapsed")
            if _sb_mode == "계획 입력":
                st.caption(
                    "셀을 클릭해 작업을 고르세요 (품번 일부를 치면 좁혀집니다, "
                    "빈 값 = 셀 비움). 수량은 **표준 UPH × 교대 시간(준비시간 차감)** "
                    "으로 자동 계산되며, 다른 수량·작업지시는 아래 '수량·메모 조정' "
                    "에서. 야간 미운영 설비의 야간 칸은 저장 때 건너뜁니다.")
                _grid = {}
                for m in _sb_rows_m:
                    row = {"설비": m["machine_id"]}
                    for d, sh in _sb_cols:
                        c = _sb_cells.get((m["machine_id"], d.isoformat(), sh))
                        row[_sb_colname[(d, sh)]] = (
                            _sb_k2lbl.get((c["plan"]["pn"], c["plan"]["process"]),
                                          c["plan"]["process"])
                            if c and c["plan"] else "")
                    _grid[m["machine_id"]] = row
                _gdf = pd.DataFrame(list(_grid.values()))
                _sb_nonce = st.session_state.setdefault("sb_nonce", 0)
                _opts = [""] + list(_sb_labels)
                with st.form("sb_grid_form"):
                    _ged = st.data_editor(
                        _gdf, hide_index=True, use_container_width=True,
                        height=min(620, 46 + 36 * len(_gdf)),
                        key=f"sb_grid_{_sb_d0}_{_sb_grp}_{_sb_nonce}",
                        column_config={
                            "설비": st.column_config.Column(disabled=True,
                                                          width="small"),
                            **{_sb_colname[c]: st.column_config.SelectboxColumn(
                                _sb_colname[c], options=_opts, width="medium")
                               for c in _sb_cols}})
                    gb1, gb2 = st.columns(2)
                    _g_save = gb1.form_submit_button("보드 저장", type="primary",
                                                     use_container_width=True)
                    _g_cancel = gb2.form_submit_button("편집 취소",
                                                       use_container_width=True)
                if _g_cancel:
                    st.session_state["sb_nonce"] = _sb_nonce + 1
                    st.rerun()
                if _g_save:
                    _n_ins, _n_upd, _n_del, _n_skip, _bad = 0, 0, 0, 0, []
                    _ins_rows = []
                    _now_s = _sb_now.now().isoformat()
                    _cur = {(s["machine_id"], str(s["plan_date"])[:10],
                             s["shift"]): s for s in _sb_sched}
                    for _bi, _brow in _ged.iterrows():
                        mid = _brow["설비"]
                        m = next((x for x in _sb_act if x["machine_id"] == mid),
                                 None)
                        if not m:
                            continue
                        for d, sh in _sb_cols:
                            _v = _brow.get(_sb_colname[(d, sh)])
                            _v = "" if _v is None or (isinstance(_v, float)
                                                      and _v != _v) else str(_v)
                            k = (mid, d.isoformat(), sh)
                            _old = _cur.get(k)
                            _old_lbl = (_sb_k2lbl.get(
                                (_old["pn"], _old["process"]), _old["process"])
                                if _old else "")
                            if _v == _old_lbl:
                                continue
                            if not _v:
                            ...
                            if not _k2:
                                _bad.append(f"{mid} {_sb_colname[(d, sh)]}: {_v}")
                                continue
                            if sh == "야간" and not m.get("night_active", True):
                                _n_skip += 1
                                continue
                    ...
                    try:
                        if _ins_rows:
                            _db.insert("production_schedule", _ins_rows)
                            _n_ins = len(_ins_rows)
                        _msg = "보드 저장 — 신규 {} · 변경 {} · 비움 {}".format(
                            _n_ins, _n_upd, _n_del)
                        if _n_skip:
                            _msg += f" · 야간 미운영 건너뜀 {_n_skip}"
                        if _bad:
                            st.warning("인식 못한 값 (목록에서 골라 주세요): "
                                       + "; ".join(_bad[:8]))
                        st.success(_msg)
                        st.session_state["sb_nonce"] = _sb_nonce + 1
                        st.rerun()
                ...
                def _sb_html(cap):
                    ...
                    h2 = "".join('<th class="d">주</th><th class="n">야</th>'
                                 for _ in _sb_days)
                    rows = []
                    _groups = sorted({m.get("group_code") or "ETC"
                                      for m in _sb_rows_m})
                    for g in _groups:
                        p, a = _sb_gl.get(g, (0, 0))
                        rows.append('<tr class="g"><th colspan="{}">{} <b>주간 '
                                    '부하 {:.0f}%</b></th></tr>'.format(
                                        2 + len(_sb_days) * 2, g,
                                        (p / a * 100) if a else 0))
                        for m in _sb_rows_m:
                            ...
                            for d in _sb_days:
                                for sh in _sch.SHIFTS:
                                    ...
                                    if sh == "야간" and not m.get("night_active",
                                                                 True):
                                        cls += " off"
                                    ...
                                    if c and c["status"] == "STOP":
                                        inner = ('<div class="job stop"><b>{}</b>'
                                                 '</div>').format(_sb_h.escape(
                                                     c["stop"] or "비가동"))
                                    elif c and c["status"] == "ACTUAL":
                                        inner = ('<div class="job actual"><b>{}'
                                                 '</b><span><span>계획 없음</span>'
                                                 '<span>실 {:,.0f}</span></span>'
                                                 '</div>').format(
                    ...
                    return ('<div class="sb-wrap"><table class="sb"><thead><tr>'
                            '<th class="m" rowspan="2">설비</th>{}<th rowspan="2">'
                            '부하</th></tr><tr>{}</tr></thead><tbody>{}</tbody>'
                            '</table></div>').format(h1, h2, "".join(rows))

                if not _sb_rows_m:
                    st.info("가동 설비가 없습니다 — 설비 탭에서 확인하세요.")
                else:
                    st.markdown(_sb_html(set()), unsafe_allow_html=True)
                    st.markdown(
                        '<div class="sb-legend">'
        ...
        # ── 접힌 영역: 작업 큐 · 기간 일괄 입력 · 수량·메모 조정 ──
        with st.expander("작업 큐 — 미배정 {}건 · 납기 위험 {}건".format(
                len(_sb_unassigned), len(_sb_late))):
            _sb_qf = st.radio("큐 필터", ["미배정", "납기 위험", "전체"],
            ...
            if not _sb_qv:
                ...
                toss_table([{
                    ...
                    "납기": q["due"] or "-",
                    "잔여": q["net"], "배정": q["alloc"],
                    "남은": max(q["net"] - q["alloc"], 0),
                    "UPH": q["uph"] or 0, "필요h": q["hours"] or 0,
                    "가능 설비": ", ".join(q["machines"][:5]) or "-",
                    "상태": ("표준없음" if q["no_std"]
                           else "완료" if q["alloc"] >= q["net"] - 1e-9
                           else "부분" if q["alloc"] > 0 else "미배정"),
                } for q in _sb_qv[:80]], badge_cols=("상태",),
                    strong_cols=("작업",),
                    num_cols=("잔여", "배정", "남은", "UPH", "필요h"))
                st.caption("잔여 = 미납 − 완성 재고 − 재공 · 필요h = 잔여 ÷ "
                           "표준 UPH · 가능 설비 = 실적이 있던 설비")

        with st.expander("기간 일괄 입력 (한 설비에 같은 작업을 여러 날)"):
            _sb_mids = [m["machine_id"] for m in _sb_rows_m] or \
                [m["machine_id"] for m in _sb_act]
            _sb_def_d = _sb_today if _sb_d0 <= _sb_today.isoformat() <= _sb_d1 \
                else _sb_days[0]
            with st.form("sb_form"):
                e1, e2 = st.columns([2, 1])
                _e_lbl = e1.selectbox("작업", list(_sb_labels), key="sb_op") \
                    if _sb_labels else None
                _e_m = e2.selectbox("설비", _sb_mids, key="sb_m") \
                    if _sb_mids else None
                e3, e4, e5 = st.columns(3)
                _e_d0 = e3.date_input("시작일", _sb_def_d, key="sb_d0")
                _e_d1 = e4.date_input("종료일", _sb_days[-1], key="sb_d1")
                ...
                _e_h = e6.number_input("가동 시간 (h/교대)", 0.5, 12.0, 9.0, 0.5,
                                       key="sb_h")
                _e_qty = e7.number_input("계획 수량 (0 = 표준으로 자동)", 0,
                                         100000, 0, 10, key="sb_qty")
                _e_wo = e8.text_input("작업지시 (선택)", key="sb_wo")
                _sb_save = st.form_submit_button("기간 저장", type="primary")
            if _sb_save:
                _k2 = _sb_labels.get(_e_lbl) if _e_lbl else None
                if not _k2 or not _e_m:
                    ...
                    _std9 = _sb_std_by.get(_k2, {})
                    _qty9 = float(_e_qty) if _e_qty > 0 else float(
                        _sch.suggest_qty(_e_h, _std9.get("uph_std"),
                                         _std9.get("setup_min") or 0))
                    ...
                    while _d <= _e_d1:
                        for sh in _sch.SHIFTS:
                            if sh not in _e_sh:
                                continue
                            if sh == "야간" and not _mrow.get("night_active",
                                                             True):
                                _n_skip += 1
                                continue
                            rec = {"pn": _k2[0], "process": _k2[1],
                                   "product_id": _sb_ops[_k2].get("product_id"),
                                   "plan_qty": _qty9, "plan_hours": float(_e_h),
                    ...
                    try:
                        if _ins_rows:
                            _db.insert("production_schedule", _ins_rows)
                            _n_ins = len(_ins_rows)
                        st.success("기간 저장 — 신규 {} · 덮어씀 {}{}".format(
                            _n_ins, _n_upd,
                            f" · 야간 미운영 건너뜀 {_n_skip}" if _n_skip
                            else ""))
                        st.rerun()
                    except Exception as e:
                        st.error(f"저장 실패: {e}")
        ...
        if _sb_sched:
            with st.expander("수량·메모 조정 (이번 주 셀 {}개)".format(
                    len(_sb_sched))):
                _adj_df = pd.DataFrame([{
                    "설비": s["machine_id"],
                    "일자": str(s["plan_date"])[5:10] + " " + (
                        "주" if s["shift"] == "주간" else "야"),
                    "작업": s["process"],
                    "수량": float(s.get("plan_qty") or 0),
                    "시간": float(s.get("plan_hours") or 0),
                    "작업지시": s.get("wo_number") or "",
                    "메모": s.get("note") or "",
                    "비움": False,
                } for s in sorted(_sb_sched, key=lambda s: (
                    s["machine_id"], str(s["plan_date"]), s["shift"]))])
                _adj_nonce = st.session_state.setdefault("sb_adj_nonce", 0)
                with st.form("sb_adj_form"):
                    _adj_ed = st.data_editor(
                        _adj_df, hide_index=True, use_container_width=True,
                        height=min(500, 46 + 36 * len(_adj_df)),
                        key=f"sb_adj_{_sb_d0}_{_adj_nonce}",
                        column_config={
                            **{c: st.column_config.Column(disabled=True)
                               for c in ("설비", "일자", "작업")},
                            "수량": st.column_config.NumberColumn(
                                min_value=0, step=10),
                            "시간": st.column_config.NumberColumn(
                                min_value=0.5, max_value=12.0, step=0.5),
                            "비움": st.column_config.CheckboxColumn()})
                    ab1, ab2 = st.columns(2)
                    _adj_save = ab1.form_submit_button("조정 저장",
                                                       type="primary",
                                                       use_container_width=True)
                    _adj_cancel = ab2.form_submit_button(
                        "편집 취소", use_container_width=True)
                if _adj_cancel:
                    st.session_state["sb_adj_nonce"] = _adj_nonce + 1
                    st.rerun()
                if _adj_save:
                    _srt = sorted(_sb_sched, key=lambda s: (
                        s["machine_id"], str(s["plan_date"]), s["shift"]))
                    _n_a, _n_d = 0, 0
                    for _bi, _brow in _adj_ed.iterrows():
                        s = _srt[int(_bi)]
                        ...
                        _new = {"plan_qty": float(_brow.get("수량") or 0),
                                "plan_hours": float(_brow.get("시간") or 9),
                                "wo_number": str(_brow.get("작업지시")
                                                 or "").strip() or None,
                                "note": str(_brow.get("메모") or "").strip()
                                or None}
                        _old = {"plan_qty": float(s.get("plan_qty") or 0),
                                "plan_hours": float(s.get("plan_hours") or 0),
                                "wo_number": s.get("wo_number") or None,
    ...
    def _po_mail_cfg():
        try:
            ...
            return mail_cfg(st.secrets)
        except Exception:
            return None

    def _po_mail_send(cfg, *, po_no, vendor, to, pdf_bytes, po_date,
                      delivery, n, total):
        """PDF 첨부 발송 + 발주를 발송(SENT)으로. (ok, 메시지)"""
        from utils.po_mail import send_po_mail, po_subject, po_body
        try:
    ...
    with tab_new:
        ...
        with st.expander("신규 거래처 등록 (수기 입력)"):
            with st.form("nv_form"):
                ...
                with ec1:
                    ...
                    nv_fax = st.text_input("팩스", key="nv_fax")
                with ec2:
                    ...
                    nv_group = st.selectbox(
                        ...
                        key="nv_group")
                    nv_type = st.selectbox("거래 구분", ["매입", "매출", "혼합"],
                                           key="nv_type")
                    ...
                    if dup:
                        ...
                        try:
                            _db.insert("vendors", [{
                                ...
                                "vendor_group": nv_group, "trade_type": nv_type,
        ...
        if vendors:
            ...
            # 최근 발주 복사·품번 일괄 추가는 제거 (2026-09-11 사용자 피드백:
            ...
            with st.expander("마스터에 없는 자재 직접 입력 (발주서에만 기재)"):
                with st.form("po_adhoc_form"):
                    ...
                    np_ = a3.number_input("단가", min_value=0, step=100,
                                          key="nx_price")
                    if st.form_submit_button("담기") and nx:
                        st.session_state.po_items.append({
                            "_uid": str(_po_uuid.uuid4())[:8],
            ...
            with st.container(border=True):
                ...
                if _por:
                    ...
                    with st.container(border=True):
                        ...
                        _mcfg = _po_mail_cfg()
                        if _mcfg and st.session_state.get("po_result_sent") != _por["po_no"]:
                            _mto = (_por.get("vendor_email") or "").strip()
                            _mk = "po_result_mail"
                            if st.button("메일로 발송 (PDF 첨부)", key=_mk,
                                         type="primary", use_container_width=True,
                                         disabled=not _mto,
                                         help=("거래처 이메일이 없습니다 — 마스터 "
                                               "관리 › 거래처 편집에서 등록"
                                               if not _mto else
                                               f"{_mto} 로 발주서 PDF 를 보내고 "
                                               "발송 처리합니다")):
                                st.session_state[f"cfm_{_mk}"] = True
                            if st.session_state.get(f"cfm_{_mk}") and confirm_gate(
                                    _mk, f"{_mto} 로 {_por['po_no']} 발주서를 "
                                         "보냅니다 — 보낸 메일은 되돌릴 수 없습니다."):
                                _ok, _info = _po_mail_send(
                                    _mcfg, po_no=_por["po_no"], vendor=_por["vendor"],
                                    to=_mto, pdf_bytes=_por["pdf"],
                                    po_date=_por.get("date"),
                                    delivery=_por.get("delivery"),
                                    n=_por["n"], total=_por["total"])
                                if _ok:
                        ...
                        if not _final:
                            ...
                            if pdf_bytes:
                                ...
                                st.session_state["po_result"] = {
                                    ...
                                    "delivery": delivery_date or None,
            ...
            if po:
                ...
                with rc2:
                    ...
                    if po["status"] == "CANCELLED":
                        ...
                        _mcfg9 = _po_mail_cfg() if po["status"] == "DRAFT" else None
                        if _mcfg9:
                            _mto9 = (full_vendor.get("email") or "").strip()
                            _mk9 = f"po_mail_{po['po_id']}"
                            if st.button("메일로 발송 (PDF 첨부)", key=_mk9,
                                         type="primary", use_container_width=True,
                                         disabled=not _mto9,
                                         help=("거래처 이메일이 없습니다 — 마스터 "
                                               "관리 › 거래처 편집에서 등록"
                                               if not _mto9 else
                                               f"{_mto9} 로 발주서 PDF 를 보내고 "
                                               "발송 처리합니다")):
                                st.session_state[f"cfm_{_mk9}"] = True
                            if st.session_state.get(f"cfm_{_mk9}") and confirm_gate(
                                    _mk9, f"{_mto9} 로 {po['po_number']} 발주서를 "
                                          "보냅니다 — 보낸 메일은 되돌릴 수 없습니다."):
                                try:
                                    _pdf9 = _re_pdf(re_po_data, re_items, re_vendor_info)
                                except Exception as _e9:
                                    _pdf9 = None
                                    st.error(f"PDF 생성 실패: {_e9}")
                                if _pdf9:
                                    _ok9, _info9 = _po_mail_send(
                                        _mcfg9, po_no=po["po_number"],
                                        vendor=po.get("_vname") or "-", to=_mto9,
                                        pdf_bytes=_pdf9, po_date=po.get("po_date"),
                                        delivery=po.get("delivery_date"),
                                        n=len(items), total=po.get("total_amount") or 0)
                                    if _ok9:
                                        st.session_state.pop("mr_cache", None)
                                        st.rerun()
                                    else:
...
# ════════════════════════════════════════════════════════════════
    ...
    pe_tab_in, pe_tab_proc, pe_tab_board, pe_tab_closed, pe_tab_trace = st.tabs(
        ["투입 등록", "공정 처리", "공정 현황판", "종결 지시서", "LOT 추적"])
    ...
    with pe_tab_closed:
        ...
        if not _cwos:
            ...
            # ── 검사 판정 취소 — 이후 처리가 없으면 취소, 있으면 사유를 보여
            # 주고 품번별 맞추기로 (2026-09-16 사용자: 관리자 한정 해제, 분기
            # 판정도 이후 처리만 없으면 되돌린다) ──
            _last = _ce[0] if _ce else None
            _why, _plan = None, None
            if (_cw.get("status") or "") not in ("CLOSED", "DONE"):
                _why = (f"지시 상태가 {status_ko(_cw.get('status'))} 이라 "
                        "검사 판정 취소 대상이 아닙니다.")
            ...
            elif not _last.get("batch_id"):
                _why = "배치 없이 처리된 구 방식 판정입니다."
            else:
                _dt = _last.get("detail") or {}
                _outq = _cfl(_dt.get("output"))
                _pb = next((b for b in _cb
                            if b.get("batch_id") == _last.get("batch_id")), None)
                # 이 판정으로 갈라진 자식 배치 — 판정 시각 이후의 SPLIT 계보
                _kids = []
                try:
                    ...
                    _lk = fetch("batch_links", "link_id,child_batch_id,qty,created_at",
                                ...
                                limit=50)
                    _lk = [l for l in _lk
                           if str(l.get("created_at") or "")[:19] >= _ev_ts]
                    _kid_ids = {l["child_batch_id"] for l in _lk}
                    _kids = [b for b in _cb if b.get("batch_id") in _kid_ids]
                except Exception:
                    _lk, _kids = [], []
                ...
                if _pb is None:
                    _why = "판정한 배치를 찾을 수 없습니다."
                elif _other:
                    _why = ("완성 LOT {} 에 출고·조정 기록 {}건이 있습니다."
                            .format(_fin_lot, len(_other)))
                else:
                    _plan = {"pb": _pb, "kids": _kids, "links": _lk,
                             "fin_lot": _fin_lot, "dt": _dt, "out": _outq}
            with st.expander("검사 판정 취소 — 예외 상황 전용"):
                if _why:
                    ...
                    if _plan["kids"]:
                        st.caption("이 판정으로 갈라진 배치 "
                                   + " · ".join(b["batch_no"] for b in _plan["kids"])
                                   + " 도 함께 지워지고 원 배치로 합쳐집니다.")
                    ...
                    if st.session_state.get(f"cfm_{_ic_k}") and confirm_gate(
                            _ic_k, f"{_cw['wo_number']} 의 검사 판정(완성 "
                                   f"{_plan['out']:,.0f})을 취소합니다 — 완성 재고에서 "
                        ...
                        try:
                            ...
                            _db.update("wo_batches", f"batch_id=eq.{_fq(_pb9['batch_id'])}",
                                       {"qty": _cfl(_pb9.get("qty")) + _kq,
                                        ...
                                        "step_status": "WAIT"})
                            _fld = {}
                            for _col9, _key9 in (("pass_qty", "pass"),
                                                 ("tokusai_qty", "tokusai"),
                                                 ("rework_qty", "rework"),
                                                 ("scrap_qty", "scrap"),
                                                 ("return_qty", "return"),
                                                 ("output_qty", "output")):
                                _fld[_col9] = max(0.0, _cfl(_cw.get(_col9))
                                                  - _cfl(_dt9.get(_key9)))
                            ...
                            _db.insert("wo_events", [{
                                ...
                                "event_type": "INSPECT_CANCEL", "qty": _plan["out"],
                                "batch_id": _pb9["batch_id"],
...
# ════════════════════════════════════════════════════════════════
    ...
    COST_FIELDS = (
        ...
        "total_sales_12m,sales_count_12m,activity_trend"        "sale_price,margin_pct_calc,"
    ...
    with tabs[1]:
        ...
        try:
            ...
            all_rows = fetch(SRC_TABLE,
                "product_id,estimated_cost_per_pc,sale_price,margin_pct_calc,total_sales_12m",
        ...
        if df_all.empty:
            ...
            # 숫자 강제 변환
            for c in ["estimated_cost_per_pc", "sale_price", "margin_pct_calc", "total_sales_12m"]:
                df_all[c] = pd.to_numeric(df_all[c], errors="coerce")

            n_total = len(df_all)
            n_has_cost = int((df_all["estimated_cost_per_pc"].fillna(0) > 0).sum())
            n_has_sale = int((df_all["sale_price"].fillna(0) > 0).sum())
            n_both = int(((df_all["estimated_cost_per_pc"].fillna(0) > 0) &
                          (df_all["sale_price"].fillna(0) > 0)).sum())
            # 마진 산출 가능한 것만으로 통계
            df_m = df_all.dropna(subset=["margin_pct_calc"])
            avg_margin = df_m["margin_pct_calc"].mean() if not df_m.empty else None
            n_neg = int((df_m["margin_pct_calc"] < 0).sum())
            n_low = int(((df_m["margin_pct_calc"] >= 0) & (df_m["margin_pct_calc"] < 10)).sum())
            ...
            k7.metric("판매 단가 있음", f"{n_has_sale:,}")
            k8.metric("원가+단가 모두 보유", f"{n_both:,}", "마진 산출 가능")
            ...
            if not df_m.empty:
                ...
                df_m["bucket"] = pd.cut(df_m["margin_pct_calc"], bins=bins, labels=labels)
            ...
            try:
                bottom = fetch(SRC_TABLE, COST_FIELDS,
                    "archived_at=is.null&margin_pct_calc=not.is.null&total_sales_12m=gt.0"
                    "&order=margin_pct_calc.asc", limit=10)
                if bottom:
                    df_b = pd.DataFrame(bottom)
                    df_b["판매가"] = df_b["sale_price"].apply(_money)
                    df_b["추정원가"] = df_b["estimated_cost_per_pc"].apply(_money)
                    df_b["마진율"] = df_b["margin_pct_calc"].apply(_pct)
            ...
            try:
                top = fetch(SRC_TABLE, COST_FIELDS,
                    "archived_at=is.null&margin_pct_calc=not.is.null&total_sales_12m=gt.0"
                    "&order=margin_pct_calc.desc", limit=10)
                if top:
                    df_t = pd.DataFrame(top)
                    df_t["판매가"] = df_t["sale_price"].apply(_money)
                    df_t["추정원가"] = df_t["estimated_cost_per_pc"].apply(_money)
                    df_t["마진율"] = df_t["margin_pct_calc"].apply(_pct)
